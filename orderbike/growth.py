...
def optimize_subtractive_growth(
    ...
    if built is True:
        edgelist = [edge for edge in G.edges if G.edges[edge]["built"] == 0]
    else:
    ...
    if metric_optimized == "directness":
        for i in tqdm.tqdm(n_iter):
            ...
            (
                ...
                dir_hist,
                cov_hist,
            ) = _make_subtractive_changes(
                G, choice, edgelist, em, sm, geom, c_hist, dir_hist, cov_hist
            )
...
def global_efficiency_subtractive_step(
    ...
    batch_choice = []  # List of corresponding edge removed
    iem = np.divide(
        np.ones(em.shape), em, out=np.zeros_like(np.ones(em.shape)), where=em != 0
    )
    if keep_connected is True:
        for edge in edgelist:
            H = G.copy()
        ...
        for edge in edgelist:
            ...
            new_em[new_sm == 0.0] = 0.0  # See above
            nge = np.sum(new_sm) / np.sum(new_em)
            batch_m.append(nge)
            batch_choice.append(edge)
    new_m = max(batch_m)
...
def coverage_subtractive_step(G, edgelist, geom, elected_nodes, keep_connected=False):
    """
    ...
    if keep_connected is True:
        for edge in edgelist:
            ...
            if _is_disconnected(H, elected_nodes) is True:
                ...
                batch_m.append(shapely.ops.unary_union(list(temp_g.values())).area)
                batch_choice.append(edge)
    else:
        for edge in edgelist:
            H = G.copy()
            ...
            temp_g.pop(edge)
            batch_m.append(
                shapely.ops.unary_union(  # See above
                    list(temp_g.values())
                ).area
            )
            batch_choice.append(edge)
    ...
    if keep_connected is True:
        for edge in edgelist:
            ...
            if _is_disconnected(H, elected_nodes) is True:
                ...
                # of the edge removed
                batch_m.append((bef_area - area) / G.edges[edge]["length"])
                batch_choice.append(edge)
    else:
        for edge in edgelist:
            H = G.copy()
...
def optimize_additive_growth(
    ...
    G = G.copy()  # Create a copy to not mutate input
    if built is True:
        actual_edges = [edge for edge in G.edges if G.edges[edge]["built"] == 1]
        edgelist = [edge for edge in G.edges if G.edges[edge]["built"] == 0]
    else:
        actual_edges = []
        edgelist = [edge for edge in G.edges]
    ...
    if built is True:
        ...
        # TODO: Build first based on metric optimized ?
        max_len_edge = max([[G.edges[edge]["length"], edge] for edge in G.edges])
        actual_edges.append(tuple(max_len_edge[1]))
        edgelist.remove(max_len_edge[1])
        c_hist.append(max_len_edge[1])
        geom[max_len_edge[1]] = shapely.ops.transform(
            project.transform, G.edges[max_len_edge[1]]["geometry"]
        ).buffer(buff_size)
        cov_hist = [
...
def directness_additive_step(G, actual_edges, edgelist, keep_connected=False):
    """
...
def relative_directness_additive_step(G, actual_edges, edgelist, keep_connected=False):
    batch_m = []
    batch_choice = []
    if keep_connected is True:
        for edge in edgelist:
...
def relative_coverage_additive_step(
    G, buff_size, project, actual_edges, edgelist, bef_area, geom, keep_connected=False
):
    """
    Find the edge to add to the actual_edges list from the final graph G
...
def random_growth(
    ...
    ValueError
        ...
        possible value. order can be additive or subtractive.
    ...
    else:  # else we need to keep the last one
...
def betweenness_growth(
    ...
    """
    Create the growth of a network G with the edges with the highest
    betweenness constructed first, in a subtractive or additive
    manner.
    ...
    folder_name : str
        ...
        the options of the functions.
    order : str
         Order of the betweenness growth,
    ...
    sorted_edges = [
        k
        for k, v in sorted(  # Sort edges by higher betweenness
    ...
    if built is True:
        ...
        if order == "additive":  # init for the subgraph
            ...
            c_hist.append(choice)  # first choice for additive
            cov_hist.append(shapely.ops.unary_union(list(geom.values())).area)
            dir_hist.append(
                metrics.directness_from_matrix(
                    metrics.get_directness_matrix_networkx(
    ...
    if keep_connected is True:
        ...
        if order == "subtractive":
            for edge in tqdm.tqdm(sub_removed_edge):
                G.remove_edge(*edge)
                G = utils.clean_isolated_node(G)
                c_hist.append(edge)
                geom.pop(edge)
